...
const SwitchLang = () => {
	...
	return (
		<div>
			<button onClick={() => setIsOpen(!isOpen)} className="m-4 flex items-center space-x-2 bg-white text-gray-600 rounded-full p-2 shadow-md hover:shadow-lg transition-shadow" aria-label="Change Language">
				{languages.find((lang) => lang.code === currentLocale)?.icon}
			</button>
			{isOpen && (
				<div className="absolute w-40 rounded-lg bg-white shadow-lg ring-1 ring-gray-200 z-10">
					{languages.map((lang) => (
